...
{
	public static var psychEngineVersion:String = '1.0 - The Virtual Update'; //This is also used for Discord RPC
	...
	var awardsWoah:FlxSprite;

	var credsToYall:FlxSprite;

	var optionsHmm:FlxSprite;

	var bonusButton:FlxSprite;

	var languageSelect:FlxSprite;
	...
	override function create()
	{
		...
		storyModeThingy.frames = Paths.getSparrowAtlas('digitalRhythmMainMenu/menu_story_mode');
		storyModeThingy.scale.set(0.3, 0.3);
		storyModeThingy.animation.addByPrefix('idle', "story_mode basic", 24);
		storyModeThingy.animation.addByPrefix('selected', "story_mode white", 24);
		...
		// Awards
		var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
	    var menuItem:FlxSprite = new FlxSprite(650, 100);
		...
		menuItem.antialiasing = ClientPrefs.globalAntialiasing;
		// menuItem.setGraphicSize(Std.int(menuItem.width * 0.20));
		menuItem.updateHitbox();


		// FlxG.camera.follow(camFollowPos, null, 1);

		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "Digital Rhythm Version " + psychEngineVersion, 12);
